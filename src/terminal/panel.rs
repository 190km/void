...
impl ScrollbarState {
    fn has_history(self) -> bool { self.history_size > 0 }

    fn total_lines(self) -> usize { self.history_size + self.screen_lines }
...
impl TerminalPanel {
    pub fn new_with_terminal(ctx: &egui::Context, position: Pos2, size: Vec2, color: Color32, cwd: Option<&std::path::Path>) -> Self {
        ...
        Self { id: Uuid::new_v4(), title, position, size, color, z_index: 0, focused: false, pty,
               last_cols: cols, last_rows: rows, selection: None, selecting: false, scroll_remainder: 0.0,
    ...
    pub fn new(title: impl Into<String>, position: Pos2, size: Vec2, color: Color32) -> Self {
        Self { id: Uuid::new_v4(), title: title.into(), position, size, color, z_index: 0, focused: false,
               pty: None, last_cols: 80, last_rows: 24, selection: None, selecting: false, scroll_remainder: 0.0,
               scrollbar_grab_offset: None }
    }
    ...
    fn terminal_body_rect(panel_rect: Rect) -> Rect {
        Rect::from_min_max(
            Pos2::new(panel_rect.min.x + 1.0, panel_rect.min.y + TITLE_BAR_HEIGHT + 1.0),
    ...
    fn scrollbar_track_rect(body: Rect) -> Rect {
        Rect::from_min_max(
            Pos2::new(body.max.x - SCROLLBAR_MARGIN - SCROLLBAR_WIDTH, body.min.y + SCROLLBAR_MARGIN),
        ...
        if let Some(state) = scrollbar_state {
            ...
            if state.has_history() {
                ...
                let thumb_hovered = scrollbar_resp.hovered()
                    && ui.input(|i| i.pointer.hover_pos()).is_some_and(|pos| thumb_rect.contains(pos));
                ...
                if scrollbar_resp.clicked_by(egui::PointerButton::Primary) {
                    ...
                    if let Some(pos) = scrollbar_resp.interact_pointer_pos() {
                        ...
                        let target_top = (pos.y - grab_offset)
                            .clamp(scrollbar_rect.top(), scrollbar_rect.bottom() - thumb_rect.height());
                        let target_offset =
                            state.offset_for_thumb_top(scrollbar_rect, thumb_rect.height(), target_top);
                ...
                if scrollbar_resp.dragged_by(egui::PointerButton::Primary) {
                    ix.clicked = true;
                    if let Some(pos) = scrollbar_resp.interact_pointer_pos() {
                        let grab_offset = self.scrollbar_grab_offset.unwrap_or(thumb_rect.height() * 0.5);
                        let target_top = (pos.y - grab_offset)
                            .clamp(scrollbar_rect.top(), scrollbar_rect.bottom() - thumb_rect.height());
                        let target_offset =
                            state.offset_for_thumb_top(scrollbar_rect, thumb_rect.height(), target_top);
